...
\begin{figure*}
	\begin{center}
         \subfloat[][]{\includegraphics[width=0.3\textwidth]{figures/Y0201_topo}}
	\subfloat[][]{\includegraphics[width=0.3\textwidth]{figures/Y0804_topo}} 
	\subfloat[][]{\includegraphics[width=0.3\textwidth]{figures/Y3216_topo}}\\
	\subfloat[][]{\includegraphics[width=0.3\textwidth]{figures/Y3217_topo}} 
	\subfloat[][]{\includegraphics[width=0.3\textwidth]{figures/tomo_topo}}
	\subfloat[][]{\includegraphics[width=0.3\textwidth]{figures/hump_vis_white}}\\
...
%	\subfloat[][]{\includegraphics[width=0.2\textwidth]{figures/tomo_moll}}
	\end{center}
	\caption{Visualisations of the topography used in the simulations: (a) $\hlm{2}{1}$;  (b) $\hlm{8}{4}$; (c) $\hlm{32}{16}$; (d)$\hlm{32}{17}$; (e) $h_t$. (f) The localized topography from \citep{tO25}.
	}
...
For this shape, which we refer to as the ``tomographic model'' and denote the corresponding shape function as $h_{t}$, we use the cluster analysis maps developed in \cite{vL12}, and consider only the areas where all of the included studies \citep{cH08,jR11,bK08,cM00,nS10} agree on the presence of an LLSVP.
...
Figures \ref{f:topo}(a)-(e) display the topographic shapes studied in the present work. For comparison, Figure \ref{f:topo}(f) displays the localized Gaussian bump investigated in \cite{tO25}.
...
\cite{mP23} and \cite{vD25} used Kaula's rule \citep{wK66} to extend the CMB topography spectrum when calculating topographic torques, and we adopt the same approach here.
...
\begin{figure}
	\begin{center}
		\subfloat[][]{\includegraphics[width=0.48\textwidth]{figures/ht_spec}}
	\end{center}
	\caption{Spectral decomposition of the tomographic shape function, $h_{t}$ at $\epsilon  = 0.20$, averaged over $m$. The solid black line shows the scaling for Kaula's rule \citep{wK66}, which we use to estimate the torques. The dashed black line shows $\ell^{-1}$ for reference.}
	\label{f:ht_spec}
\end{figure}
...
\begin{figure}
	\begin{center}
        \subfloat[][]{\includegraphics[width=0.19\textwidth]{figures/0x10Y0201_geo_contours}}
	\subfloat[][]{\includegraphics[width=0.19\textwidth]{figures/0x10Y0804_geo_contours}}
	\subfloat[][]{\includegraphics[width=0.19\textwidth]{figures/0x10Y3216_geo_contours}}
	\subfloat[][]{\includegraphics[width=0.19\textwidth]{figures/0x10Y3217_geo_contours}}
	\subfloat[][]{\includegraphics[width=0.19\textwidth]{figures/0x10tomo_geo_contours}} \\
...
%	\subfloat[][]{\includegraphics[width=0.25\textwidth]{figures/0x10tomo_equator_uphi_avg}} 	
	%%
	\end{center}
	\caption{Geostrophic contours and impact on time-averaged flow. Geostrophic contours for (left to right) $\hlm{2}{1}$, $\hlm{8}{4}$, $\hlm{32}{16}$, $\hlm{32}{17}$, and $h_{t}$ for $\epsilon  = 0.1$. }
	\label{f:t_geo_c}
\end{figure}
...
\begin{equation}
	...
	\label{e:meq}
\end{equation}
The weighted wavenumber is similar to the mean spherical harmonic degree used in \cite{uC06} (also \cite{jN24}), however it is defined for the order rather than the degree.
