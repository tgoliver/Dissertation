...
Equatorially anti-symmetric topography, corresponding to spherical harmonics where $\ell-m$ is odd, generates smaller amplitude and larger wavenumber deviations than equatorially symmetric topography. 
...
In the core, the convective scales are likely much smaller than the scales of topography, so, for core-like parameters, we may not expect this second effect to significantly increase heat and momentum transport. 
...
The $\hlm{32}{16}$ and $\hlm{32}{17}$ topographies show elevated kinetic energy at short wavelengths, which we attribute to the topographic wavenumber being near the critical wavenumber $m_{c}$. 
...
In the $\hlm{32}{16}$ case the pressure fluctuations in the torque yielding harmonic decreases with increasing $\epsilon$ for $\epsilon >0.02.$ We attribute this decrease to the convective instability preferentially locking to the mode in phase with the topography. 
...
The kinetic helicity of the flow, defined as the dot product of the velocity and vorticity vectors, is known to be a crucial ingredient for the generation of global scale magnetic field \citep{eP55}. In spheres it is observed to be largely antisymmetric about the equator but if topography disrupts this behaviour, the resultant magnetic fields will likely exhibit significant inhomogeneity.
 ...
 As addressed in seismological investigations, the shape of the CMB is not well constrained. We have employed a simple adaptation of the `cluster analysis' or `voting map' procedure discussed in \cite{sC16}, \cite{pK21}, and \cite{vL12} to construct what we have referred to as our tomographic model. 
 ...
 While the topographic torques for this particular model exhibited similar behavior in comparison to the other models (with the exception of the $\hlm{32}{16}$ model), the changes in flow patterns were minimal. 
 Future efforts will examine the role of less `edgy' topography, including models inspired from mantle convection studies \citep[e.g.][]{tL07,tL10}.
 ...
 In the Earth's core it is likely that mantle convection is necessary to sustain CMB topography over geologic timescales.
...
\cite{tG25} and \cite{jK22} investigated rotating convection in a thin shell in which the effects of melting are modeled.
They find that convection can directly generate topography. 
