...
Convection plays an important role in the dynamics of many planets and stars, where it serves as the power source for sustaining the magnetic fields of the planets \citep{sS10,cJ11b,pR13,jmA15} and the Sun \citep{pC14}. Convection may also be a possible driving mechanism for the observed large scale zonal winds  (eg. \citep{mH16}) and vortices (eg. \citep{lS22}) on the giant planets. The flows in these natural systems are strongly forced and turbulent and can be constrained by the Coriolis force. 
...
\ee
where $U$ is a characteristic flow speed. In addition, the Reynolds number is given by $Re = U H/ \nu = Ro/Ek$; QG turbulence is characterized by $Re \gg 1$, and therefore $Ek \ll Ro \ll 1$. As an example, for the Earth's outer core estimates suggest $Ro = O(10^{-7})$, $Ek = O(10^{-15})$ and $Re = O(10^8)$ (eg. \citep{pR13}). 
...
In the present investigation we employ \textcolor{black}{a modified version of} the asymptotically reduced form of the governing equations given by Ref.~\citep{mS06}. When non-dimensionalized using the small-scale viscous diffusion speed $\nu/\ell^*_\nu$, where $\ell^*_\nu = H Ek^{1/3}$, \textcolor{black}{and temperature scale $\Delta T$,} these equations take the form
	\be
...
%\textcolor{black}{The term in angled brackets denotes a depth average and is the implemented modification intended to suppress the largest scale vortex. Simulations that were performed ``with an LSV'' do not include this modification.}
%\textcolor{black}{We thank an anonymous referee \textcolor{red}{What is the etiquette here?-TO} for raising an issue concerning our implementation of a previous method for removing the barotropic component. Upon review of the code, we discovered a mistake that has since been removed.}
...
\end{center}
		%\end{footnotesize}
\caption[Details of the numerical simulations for Chapter \ref{Oliver_2023} (with LSV)]{Details of the numerical simulations for cases in which depth-invariant flows are not suppressed. See Table \ref{tab:run} for specifics. Runs denoted with an asterisk ($*$) are from Ref.~\cite{sM21}.}
\end{table}
...
Nevertheless, we conclude that our results for $\vartheta_{rms}$ in the thermal boundary layer are consistent with the theoretically deduced $\Rat^{7/8}$ scaling discussed in \textcolor{black}{Ref.~\cite{kJ12b}.}
As previously reported in \textcolor{black}{Ref.~\citep{mS06}}, we observe saturation of the interior mean temperature gradient with a value of $-\partial_{Z}\overline\Theta \approx 0.4$-$0.5$ (Figure \ref{F:temp}\protect\subref{dz_temp}). 
...
%In \protect\subref{temp_da}, \protect\subref{temp_max_da}, \ding{110} are computed at the edge of the thermal boundary layer, defined as the location of max($\vartheta_{rms}$), black \ding{108} are averaged over the entire depth and orange \ding{115} are averaged over the mid-depth range $0.4 < Z < 0.6$.
}
\label{F:temp}
\end{figure}
...
%\textcolor{black}{For $Nu$, the data suggests this asymptotic scaling, but it is possible that a large enough $\Rat$ regime has not been reached. 
